...
class SlideWindow:
    ...
    def slidewindow(self, img):
        ...
        good_left_inds = ((nonzerox >= 35) & (nonzeroy >= nonzerox*0.4 + 340) & (nonzerox <= 250)).nonzero()[0]
        good_right_inds = ((nonzerox >= 420) & (nonzeroy >= nonzerox*(-0.3) + 540) & (nonzerox <= 620)).nonzero()[0]
        ...
        if line_flag != 3:
            ...
            # 찾은 좌표들을 circle로 표시
            if line_flag == 1:
                # 차선-가운데 폭이 160정도 (width * 0.26)
                cv2.circle(out_img, (x_current + int(width * 0.26), y_current), 5, (0, 0, 255), -1)

                for i in range(len(good_left_inds)):
                    cv2.circle(out_img, (nonzerox[good_left_inds[i]], nonzeroy[good_left_inds[i]]), 1, (0, 255, 0), -1)

                # nonzeroy min값과 max값 추출
                y1 = int(np.min(nonzeroy[good_left_inds]))
                y2 = int(np.max(nonzeroy[good_left_inds]))

                # y1, y2에 해당하는 x값들의 평균을 구함 (오차값 허용)
                x1 = int(np.mean(nonzerox[(nonzeroy<=y1+15) & (nonzeroy>=y1-15) & ((nonzerox >= 35) & (nonzeroy >= nonzerox*0.4 + 340) & (nonzerox <= 250))]))
                x2 = int(np.mean(nonzerox[(nonzeroy<=y2+10) & (nonzeroy>=y2-10) & ((nonzerox >= 35) & (nonzeroy >= nonzerox*0.4 + 340) & (nonzerox <= 250))]))

                cv2.line(out_img, (x1,y1), (x2,y2), (0,0,255), 3)


            else:
                cv2.circle(out_img, (x_current - int(width * 0.22), y_current), 5, (0, 0, 255), -1)
            ...
            for window in range(0, nwindows):
                ...
                if line_flag == 1:
                    ...
                    if win_y_low >= 380 and win_y_low < 420:
                        # 0.165 is the half of the road(0.33)
                        # 현재 car의 위치를 계산
                        x_location = x_current + int(width * 0.26)
                    ...
                    if win_y_low >= 380 and win_y_low < 420:
                        # 0.165 is the half of the road(0.33)
                        x_location = x_current - int(width * 0.22)
